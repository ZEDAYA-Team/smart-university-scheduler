...
export default function Dashboard() {
  ...
  const [summaryData, setSummaryData] = useState<{
    ...
    average_room_utilization: string;
    lecturers_on_schedule: number;
  ...
  useEffect(() => {
    async function fetchSummary() {
      try {
        const res = await fetch("http://127.0.0.1:8000/api/dashboard/summary?semester_id=1");
        if (res.ok) {
  ...
  function exportReport(name: string) {
    ...
    window.open(`http://127.0.0.1:8000/api/reports/${formattedName}.csv`, "_blank");
    setExported(name);
  ...
  return (
    <main className="shell">
      <aside className={mobileOpen ? "sidebar open" : "sidebar"}>
        ...
        <nav>{nav.map((item, index) => <button key={item} onClick={() => { setActive(item); setMobileOpen(false); }} className={active === item ? "nav-item selected" : "nav-item"}>{[<LayoutDashboard key="d"/>, <CalendarDays key="t"/>, <AlertTriangle key="c"/>, <FileText key="r"/>][index]}{item}{item === "Conflicts" && <em>{summaryData ? summaryData.open_conflicts : 3}</em>}</button>)}</nav>
      ...
      <section className="content">
        ...
        <div className="page-title"><div><p className="eyebrow">OVERVIEW</p><h1>{active === "Dashboard" ? "Dashboard" : active}</h1><p className="sub">CNCS pilot · Addis Ababa University</p></div><button className="primary" onClick={() => exportReport("room_utilization")}><Download size={17}/>Export summary</button></div>

        {/* Live Metrics Grid hooked to FastAPI */}
        <section className="metrics">
          <Metric 
            icon={<CalendarDays/>} 
            label="Scheduled sessions" 
            value={summaryData ? String(summaryData.scheduled_sessions) : "..."} 
          ...
          <Metric 
            ...
            value={summaryData ? `${summaryData.average_room_utilization}%` : "..."} 
            detail="Across 18 active rooms" 
          />
          <Metric 
            icon={<Users/>} 
            label="Lecturers on schedule" 
            value={summaryData ? String(summaryData.lecturers_on_schedule) : "..."} 
            detail="4 are near load limit" 
          />
          <Metric 
            icon={<AlertTriangle/>} 
            label="Open conflicts" 
            value={summaryData ? String(summaryData.open_conflicts) : "..."} 
        ...
        <section className="grid two-col lower">
          ...
          <article className="card"><div className="card-head"><div><h2>Lecturer workload</h2><p>Teaching load status for this semester</p></div><button className="text-button" onClick={() => exportReport("lecturer_workload")}>Full report</button></div><div className="workload"><Workload name="Dr. Meron Assefa" course="CS · 16 / 18 hours" percent={89}/><Workload name="Dr. Solomon Tadesse" course="Mathematics · 12 / 16 hours" percent={75}/><Workload name="Ms. Selamawit Worku" course="Physics · 10 / 14 hours" percent={71}/></div></article>
        </section>
